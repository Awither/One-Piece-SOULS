...
function initSoulsPanel() {
  ...
  if (addBtn) {
    addBtn.addEventListener("click", () => {
      ...
      // Souls aren't needed in playable view directly, so no renderPlayableView here
    });
  }
...
/**
 * Build the shared "Assign To" options list used by abilities.
 */
function getAssignOptions() {
  const options = [
    { value: "general", label: "General / Party" }
  ];
...
/**
 * Fill an <select> with Homie/Domain/General assignment options.
 */
function fillAssignSelect(selectEl, currentValue) {
  if (!selectEl) return;
  const options = getAssignOptions();
  const current = currentValue || "general";
  ...
  selectEl.value = options.some((o) => o.value === current) ? current : "general";
}

/**
 * Refresh the assign selects on the Ability panel (manual + AI) when
 * Homies / Domains change.
 */
function refreshAbilityAssignSelects() {
  const manualAssign = $("#manual-ability-assign");
  const aiAssign = $("#ai-ability-assign");
  if (manualAssign) fillAssignSelect(manualAssign, manualAssign.value);
  if (aiAssign) fillAssignSelect(aiAssign, aiAssign.value);
}
...
function refreshHomieOptions() {
  ...
  // Homies changed, update ability assignment options too
  refreshAbilityAssignSelects();
}
...
function refreshDomainOptions() {
  ...
  // Domains changed, update ability assignment options too
  refreshAbilityAssignSelects();
}
  ...
  function renderHomies() {
    ...
    Homies.filter((h) =>
      ...
      delBtn.addEventListener("click", () => {
        ...
        renderHomies();
        refreshHomieOptions();
        renderPlayableView(); // keep dashboard live
      });
    ...
    refreshHomieOptions();
    renderPlayableView(); // if homies changed, playable view updates
  }
...
function initAbilitiesPanel() {
  ...
  // On init, build the assign selects once
  refreshAbilityAssignSelects();
  ...
  if (manualSaveBtn && manualName) {
    manualSaveBtn.addEventListener("click", () => {
      ...
      const assignValue = manualAssign?.value || "general";
      ...
      const ability = {
        ...
        role: null,
        assignToType: inferAssignType(assignValue),
        assignToId: assignValue,
        rawText
      };
  ...
  if (aiGenerateBtn && aiPrompt) {
    aiGenerateBtn.addEventListener("click", async () => {
      ...
      const assignValue = aiAssign?.value || "general";

      LastAbilityAIInput = { concept, power, role, assignValue };
  ...
  if (aiRerollBtn) {
    aiRerollBtn.addEventListener("click", () => {
      ...
      const { concept, power, role, assignValue } = LastAbilityAIInput;
      if (aiPrompt && concept) aiPrompt.value = concept;
      if (aiPower && power) aiPower.value = power;
      if (aiRole && role) aiRole.value = role;
      if (aiAssign && assignValue) aiAssign.value = assignValue;
      aiGenerateBtn?.click();
    });
  }
  ...
  function renderAbilityList() {
    ...
    Abilities.filter((a) =>
      ...
      // --- On-card assignment dropdown ---
      const assignRow = document.createElement("div");
      assignRow.style.marginTop = "0.4rem";
      assignRow.style.display = "flex";
      assignRow.style.alignItems = "center";
      assignRow.style.gap = "0.35rem";

      const assignLabel = document.createElement("div");
      assignLabel.style.fontSize = "0.72rem";
      assignLabel.style.color = "#a0a3d0";
      assignLabel.textContent = "Assigned to:";

      const assignSelect = document.createElement("select");
      assignSelect.style.fontSize = "0.75rem";
      assignSelect.style.padding = "0.25rem 0.4rem";
      assignSelect.style.borderRadius = "999px";
      assignSelect.style.border = "1px solid rgba(132,157,255,0.7)";
      assignSelect.style.background = "rgba(7,9,36,0.96)";
      // ability.assignToId holds the raw select value like "general", "homie:<id>"...
      fillAssignSelect(assignSelect, ability.assignToId || "general");

      assignSelect.addEventListener("change", () => {
        const val = assignSelect.value;
        ability.assignToType = inferAssignType(val);
        ability.assignToId = val;
        saveAll();
        renderAbilityList();
        renderPlayableView();
      });

      assignRow.append(assignLabel, assignSelect);
  ...
  initAbilitiesPanel.renderAbilityList = renderAbilityList;
      ...
      try {
        ...
        if (parts.length > 0) {
          parts.forEach((chunk) => {
            ...
            const rawText =
              "Name: " +
              chunk; /* ensures parseAbilityText has the same info */
...
function renderPlayableView() {
  ...
  Homies.forEach((h) => {
    ...
    coreLine.innerHTML =
      ...
      (h.notes ? `<div><b>Notes:</b> ${h.notes}</div>` : "");
    ...
    Abilities.filter(
      ...
      addMetaItem("Action", metaObj.action);
      addMetaItem("Range", metaObj.range);
      addMetaItem("Target", metaObj.target);
      addMetaItem("Save / DC", metaObj.save);
      addMetaItem("Damage", metaObj.damage);

      smallCard.appendChild(metaRow);
  ...
  Domains.forEach((d) => {
    ...
    // Domain-assigned abilities
    const domainAbilHeader = document.createElement("div");
    domainAbilHeader.style.fontSize = "0.8rem";
    domainAbilHeader.style.fontWeight = "600";
    domainAbilHeader.style.marginTop = "0.3rem";
    domainAbilHeader.textContent = "Domain Abilities";
    body.appendChild(domainAbilHeader);

    const domainAbilList = document.createElement("div");
    domainAbilList.className = "card-list ability-card-list";

    Abilities.filter(
      (a) =>
        a.assignToType === "domain" &&
        a.assignToId === `domain:${d.id}`
    ).forEach((a) => {
      const metaObj = parseAbilityText(a.rawText || a.text || "");
      const card = document.createElement("div");
      card.className = "ability-card";
      ...
      domainAbilList.appendChild(card);
    });

    if (!domainAbilList.children.length) {
      const noAbil = document.createElement("div");
      noAbil.style.fontSize = "0.74rem";
      noAbil.style.color = "#9ba0e6";
      noAbil.textContent = "No domain abilities assigned yet.";
      domainAbilList.appendChild(noAbil);
    }

    body.appendChild(domainAbilList);

    // Lair actions
    const lairHeader = document.createElement("div");
    lairHeader.style.fontSize = "0.8rem";
    lairHeader.style.fontWeight = "600";
    lairHeader.style.marginTop = "0.6rem";
